...
const signupUser = async(request, response) => {
    ...
    try{
        ...
        const hashedPassword = await bcrypt.hash(userData.password, 10)
            ...
            const {password, _id, createdAt, __v, updatedAt, reviews, cart, favourites, ...user} = newUser?._doc
            return response.status(201).send({ status: "success", code: 201, message: "User registered successfully", userData: user})
    }
    catch(error){
...
const loginUser = async(request, response) => {
    ...
    try{
        ...
        if(await bcrypt.compare(userCredentials.password, validUser.password)){
            ...
            const {password, _id, createdAt, __v, updatedAt, reviews, cart, favourites, ...userData} = validUser?._doc
            return response.status(200).send({ status: "success", code: 200, message: "Loggin successfull", userData })
        }
        else{
